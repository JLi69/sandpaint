...
pub fn inside_circle(
	...
	(circle_y - y) * (circle_y - y) + 
		(circle_x - x) * (circle_x - x) < (radius * radius)
}
...
impl SandGrid {
    ...
    pub fn new(w: usize, h: usize) -> Self {
        SandGrid {
            grid: vec![SandParticle { sand_type: Sand::Air, updated: false }; w * h],
            width: w,
            height: h,
        }
    }
	...
	fn update_pixel(
	    ...
	    match self.get_sand(x, y) {
	        ...
	        Sand::Oil => { 
	            update_sand::transform_from_neighbors(x, y, Sand::Lava, Sand::Fire, self, 1, 4, 0.2);
				update_sand::update_liquid(x, y, self, sand_property);
	        }
	        Sand::Lava => {
	            update_sand::update_liquid(x, y, self, sand_property);
	        ...
	        Sand::Stone => {
	            move_sand::fall_down(x, y, self, sand_property);
	        }
	        Sand::Wood => {
	            update_sand::transform_from_neighbors(x, y, Sand::Lava, Sand::Fire, self, 1, 4, 0.2);
	        }
