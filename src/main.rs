...
mod sand; use sand::{sand_properties::SandSimulationProperties, Sand, SandGrid};
...
fn change_brush_size(brush_radius: u32,
					 min_radius: u32,
					 max_radius: u32,
					 scroll_direction: i32) -> u32 {
...
fn display_sand_select(canvas: &mut Canvas<Window>,
					   sand_menu: &[Sand],
					   selected_ind: usize) -> Result<(), String> {
	//Display the menu
    canvas.set_draw_color(Color::RGB(64, 64, 64));
    canvas.fill_rect(Rect::new(0, 0, 800, 16)).map_err(|e| e.to_string())?;
...
fn update_sand(sand_grid: &mut SandGrid, 
...
fn display_brush(pixels: &mut [u8], 
			  mousex: isize, 
			  mousey: isize, 
			  radius: u32,
			  sand_grid: &SandGrid) {
	for y in (mousey - radius as isize)..(mousey as isize + radius as isize) {
        for x in (mousex - radius as isize)..(mousex as isize + radius as isize) {
            ...
            if !sand::inside_circle(mousex as i32, mousey as i32, radius as i32, x as i32, y as i32) {
                continue;
            }
...
fn mouse_place_sand(event_pump: &EventPump, 
...
fn main() -> Result<(), String> {
    ...
    while !sim_clock.quit {
		...
		mouse_place_sand(&event_pump, &mut sand_grid, &sand_menu, selected_sand_ind, radius);
        ...
        //Display sand grid  
        canvas.clear();
		sand_texture.with_lock(None, |pixels: &mut [u8], _pitch: usize| {
        ...
        .map_err(|e| e.to_string())?;
        canvas.copy(&sand_texture, None, None).map_err(|e| e.to_string())?;
		//Display Menu
