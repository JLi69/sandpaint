use crate::sand::{move_sand, Sand, SandGrid, SandProperties};
...
pub fn update_particle(
    ...
    if y == sand_grid.height - 1 {
        if properties
            .can_replace
            .contains(&future_sand[y * sand_grid.width + x])
        {
...
pub fn explode(
    ...
    while angle < 3.14159 * 2.0 {
        ...
        while (posx * posx + posy * posy).sqrt() < radius as f64 {
            ...
            if properties
                .can_replace
                .contains(&sand_grid.grid[trans_y * sand_grid.width + trans_x])
            {
...
pub fn update_fire(
    ...
    for yoff in -2isize..2isize {
        for xoff in -2isize..2isize {
            ...
            if properties
                .can_replace
                .contains(&sand_grid.grid[posy * sand_grid.width + posx])
